...
if [[ -z "$QUERY_STRING" ]]; then
    ...
    if [[ $(sqlite3 ${BADMINTON_ENV:-prod}.db "select count(1) from team_player where player_id = $PLAYER_ID") -gt 0 ]]; then
...
<form>
<p>
Are you sure you want to delete the player? ($(sqlite3 ${BADMINTON_ENV:-prod}.db "$(printf 'select * from player where id = %s' $PLAYER_ID)"))</p>
